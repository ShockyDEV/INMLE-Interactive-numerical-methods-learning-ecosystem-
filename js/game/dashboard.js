...
(function (NS) {
  ...
  NS.views.progress = {
    mount: function (root) {
      ...
      tit.appendChild(el('p', 'eyebrow', 'Autoevaluación'));
      ...
      c1.appendChild(el('p', null, '<span class="dash-grande">' + g.puntos + '</span> puntos'));
      c1.appendChild(el('p', 'met-desc', g.insignias + ' de ' + NS.achievements.lista.length + ' insignias · ' + g.maestros + '/11 métodos a nivel Experto o más'));
      ...
      Object.keys(NS.registry).forEach(function (mid) {
        ...
        c2.appendChild(fila(m.nombre, nivel / 4, NIVELES[nivel]));
      });
      grid.appendChild(c2);
      ...
      Object.keys(NS.registry).forEach(function (mid) {
        ...
        const p = d.practica;
        c3.appendChild(fila(
          NS.registry[mid].nombre,
          p.primerIntento / p.intentos,
          Math.round(p.primerIntento / p.intentos * 100) + '% de ' + p.intentos
        ));
      });
      if (!hayPractica) c3.appendChild(el('p', 'met-desc', 'Aún no has practicado: entra en cualquier método y abre la pestaña Practicar.'));
      grid.appendChild(c3);
      ...
      c6.appendChild(el('h3', null, 'Insignias'));
      NS.achievements.lista.forEach(function (a) {
        const tiene = !!st.logros[a.id];
        const chip = el('span', 'insignia ' + (tiene ? 'lograda' : 'pendiente'));
        chip.innerHTML = '<span class="ini">' + a.nombre.charAt(0) + '</span> <span><b>' + a.nombre + '</b> · ' + a.desc + '</span>';
